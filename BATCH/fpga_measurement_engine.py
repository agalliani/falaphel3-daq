...
### METODI DI INIEZIONE E SWEEP PIXEL ###
    ...
    def perform_sweep(self, port: str, baud: int, sweep_params: Dict[str, int], 
                      binary_command_params: Dict[str, int], pixel_config_params: Dict[str, int], isMatrixScan: bool = False, progress_reporter: Optional[Any] = None, sweep_task_id: Optional[TaskID] = None) -> float:
    ...
    def perform_sub_matrix_scan(self, port: str, baud: int,
            ...
            try:
                ...
                # --- RIMUOVI IL TASK SWEEP PIXEL UNA VOLTA FINITO LO SCAN ---
                if progress_reporter and scan_task_id:
                    progress_reporter.remove_task(scan_task_id)
